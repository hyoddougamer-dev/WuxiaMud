...
/** Convenience: a bowed stroke between two points, swept in one call. */
export function strokeBetween(
  ...
  return sweep(bowedSpine(from, to, bow, segments), width, rng, jitter)
}

/**
 * An ink splatter: a ring of droplets with wildly uneven sizes and distances.
 * Used for impacts and damage instead of red pixels.
 */
export function splatter(
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Squared distribution clusters most drops near the impact point, with a
    // few thrown far — even spacing looks like a machine, not a splash.
    const dist = Math.pow(rng.next(), 2) * spread
    drops.push({
      x: Math.cos(angle) * dist,
      y: Math.sin(angle) * dist,
      r: rng.range(0.6, 3.2) * (1 - dist / spread / 1.6),
    })
  }
  return drops
}
